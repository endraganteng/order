...
@section('title', 'Tambah Supplier - Admin')

@section('content')
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <h2 style="color: var(--color-text-dark, #333);">Tambah Supplier Baru</h2>
        ...
        <form method="POST" action="{{ route('admin.suppliers.store') }}">
            ...
            <div style="margin-bottom: 20px;">
                ...
                <textarea name="address" rows="3"
                    style="width: 100%; padding: 10px; border: 2px solid #e0e0e0; border-radius: 4px; resize: vertical;">{{ old('address') }}</textarea>
                @error('address')
                    <span style="color: #dc3545; font-size: 12px;">{{ $message }}</span>
                @enderror
            </div>
