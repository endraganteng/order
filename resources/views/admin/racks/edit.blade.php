        ...
        <form method="POST" action="{{ route('admin.racks.update', $rack['id']) }}">
            ...
            <div class="form-group">
                ...
                <select id="rack-type" name="rack_type" class="form-input" required>
                    <option value="storage" {{ old('rack_type', $rack['rack_type'] ?? 'storage') === 'storage' ? 'selected' : '' }}>📦 Storage (Gudang/Stok)</option>
                    <option value="display" {{ old('rack_type', $rack['rack_type'] ?? 'storage') === 'display' ? 'selected' : '' }}>🏪 Display (Etalase/Customer)</option>
                </select>
