...
<head>
    ...
    <title>Jadwal Saya - {{ $waiterName }}</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .today-card.is-full { background: linear-gradient(135deg, #fef2f2 0%, #fee2e2 100%); border-left: 5px solid #dc2626; }
        .today-card.is-pagi { background: linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%); border-left: 5px solid #2563eb; }
        .today-card.is-sore { background: linear-gradient(135deg, #ecfdf5 0%, #d1fae5 100%); border-left: 5px solid #059669; }
        .today-card__label { font-size: 0.75rem; text-transform: uppercase; color: #64748b; font-weight: 600; letter-spacing: 0.5px; }
        .today-card__day { font-size: 1.05rem; font-weight: 700; margin-top: 2px; color: #1e293b; }
        ...
        .today-card.is-full .today-card__shift { color: #991b1b; }
        .today-card.is-pagi .today-card__shift { color: #1e40af; }
        .today-card.is-sore .today-card__shift { color: #065f46; }
        ...
        .week-nav__btn:disabled, .week-nav__btn[aria-disabled="true"] { opacity: 0.5; pointer-events: none; }
        ...
        .day-row__day { font-weight: 700; font-size: 0.92rem; color: #1e293b; }
        .day-row__date-text { font-size: 0.72rem; color: #64748b; }
        .day-row.is-today .day-row__day::after {
            content: ' ← Hari ini';
            font-size: 0.7rem;
            color: #d97706;
            font-weight: 600;
        ...
        .shift-pill.shift-full { background: #fef2f2; color: #991b1b; border: 1px solid #fca5a5; }
        .shift-pill.shift-pagi { background: #eff6ff; color: #1e40af; border: 1px solid #93c5fd; }
        .shift-pill.shift-sore { background: #ecfdf5; color: #065f46; border: 1px solid #6ee7b7; }
        ...
        .summary-item .label { font-size: 0.7rem; color: #64748b; text-transform: uppercase; letter-spacing: 0.3px; }
        .summary-item .value { font-size: 1.15rem; font-weight: 800; margin-top: 2px; color: #1e293b; }
...
<div class="container">
    @if(! $isRetailEmployee)
        <div class="empty-state">
            <div class="icon">📋</div>
            <h2>Jadwal Retail Belum Tersedia</h2>
            <p>Anda belum terdaftar sebagai karyawan retail di sistem jadwal. Hubungi admin untuk info lebih lanjut.</p>
        </div>
    @elseif(! $schedule)
        <div class="empty-state">
            <div class="icon">⚠️</div>
            <h2>Schedule Tidak Tersedia</h2>
            <p>Terjadi masalah saat memuat jadwal. Coba refresh atau hubungi admin.</p>
        </div>
    @else
        @php
        ...
        @if($isCurrentWeek && $today)
            <div class="today-card {{ $todayClass }}">
                ...
                @else
                    <div class="today-card__time">Selamat istirahat ya</div>
                @endif

                {{-- Break info kalau hari ini 3-orang day --}}
                @php
                    $threePersonDays = ['thursday', 'friday', 'saturday', 'sunday'];
                    $todayDayKey = strtolower(\Carbon\Carbon::parse($today['date'])->englishDayOfWeek);
                    $isThreePerson = in_array($todayDayKey, $threePersonDays, true) && $today['shift'] !== 'LIBUR';
                    $breakSlot = match($today['shift']) {
                        'PAGI' => '12:00 – 13:00',
                        'FULL' => '13:00 – 14:00',
                        'SORE' => '14:00 – 15:00',
                        default => null,
                ...
                @endphp
                @if($isThreePerson && $breakSlot)
                    <div class="today-card__break">☕ Istirahat: <strong>{{ $breakSlot }}</strong></div>
                @elseif($today['shift'] !== 'LIBUR' && ! $isThreePerson)
                    <div class="today-card__break">☕ Istirahat fleksibel di toko (cuma 2 orang masuk)</div>
                @endif
            </div>
        @endif
        ...
        {{-- Week nav --}}
        <div class="week-nav">
            <a href="{{ route('waiter.jadwal', ['week_start' => $prevWeek]) }}" class="week-nav__btn">←</a>
            <div class="week-nav__title">
                Minggu {{ $schedule['week_iso'] }}
                @if($schedule['is_holder'])
                    <span class="holder-badge">4× FULL</span>
                @endif
            </div>
            <a href="{{ route('waiter.jadwal', ['week_start' => $nextWeek]) }}" class="week-nav__btn">→</a>
        </div>
        ...
        <div class="week-card">
            ...
            @foreach($schedule['days'] as $day)
                ...
                @endphp
                <div class="day-row {{ $day['is_today'] ? 'is-today' : '' }} {{ $day['is_weekend'] ? 'is-weekend' : '' }}">
